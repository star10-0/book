...
export function HeroSection() {
  return (
    <section className="rounded-3xl bg-gradient-to-l from-slate-900 via-indigo-800 to-indigo-600 p-6 text-white shadow-lg sm:p-10 lg:p-12" aria-labelledby="hero-title">
      <p className="inline-flex rounded-full border border-white/25 bg-white/10 px-3 py-1 text-xs font-semibold text-indigo-100">عروض رقمية يومية</p>
      <h1 id="hero-title" className="mt-3 text-3xl font-bold leading-tight sm:text-4xl lg:text-5xl">تسوّق كتب عربية رقمية بسهولة</h1>
      <p className="mt-4 max-w-2xl text-base leading-8 text-indigo-100 sm:text-lg">
        اكتشف أحدث الكتب، قارن بين الشراء والاستئجار، وابدأ القراءة فورًا من مكتبتك الشخصية.
      </p>
      <div className="mt-6 grid gap-3 text-sm sm:grid-cols-3">
        <p className="rounded-xl bg-white/10 px-3 py-2">✓ مكتبة عربية متجددة</p>
        <p className="rounded-xl bg-white/10 px-3 py-2">✓ شراء واستئجار في صفحة واحدة</p>
        <p className="rounded-xl bg-white/10 px-3 py-2">✓ وصول فوري بعد تأكيد الدفع</p>
      </div>
...
export function CategoriesPreviewSection({ categories }: { categories: CategoryPreviewItem[] }) {
  return (
    <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8">
      <div className="flex items-center justify-between gap-3">
        <h2 className="text-2xl font-bold text-slate-900">تسوّق حسب التصنيف</h2>
        <Link href="/books" className="text-sm font-semibold text-indigo-600 hover:text-indigo-700">
          كل التصنيفات
        </Link>
      </div>
      ...
      {categories.length === 0 ? (
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {categories.map((category) => (
            <article key={category.name} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:shadow-sm">
              ...
              <Link href={`/books?q=${encodeURIComponent(category.name)}`} className="mt-3 inline-block text-xs font-semibold text-indigo-700 hover:text-indigo-600">
                تصفّح هذا التصنيف
              </Link>
            </article>
          ))}
