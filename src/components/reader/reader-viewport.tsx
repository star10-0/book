...
export function ReaderViewport({
  ...
  if (source.kind === "TEXT") {
    return (
      <article className={`max-h-[88vh] overflow-y-auto whitespace-pre-wrap rounded-xl border p-6 leading-8 ${palette.wrapper} ${palette.frame}`}>
        {source.textContent}
      </article>
    );
  }
    ...
    return (
      <div className={`relative overflow-hidden rounded-xl border ${palette.frame}`}>
        <iframe
          ...
          className={`h-[88vh] w-full ${palette.wrapper}`}
        />
        <canvas
          ref={canvasRef}
  ...
  return (
    <article className={`max-h-[88vh] overflow-y-auto rounded-xl border p-6 leading-8 ${palette.wrapper} ${palette.frame}`}>
